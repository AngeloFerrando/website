...
public class VCWorld extends Environment {
  ...
  public void init(String[] args) {
    ...
    dirty = new boolean[n][n];
    lPos = new Literal[n][n];
    for(int i = 0; i < n; i++){
      for(int j = 0; j < n; j++){
        dirty[i][j] = true;
        lPos[i][j] = ASSyntax.createLiteral("pos", ASSyntax.createNumber(j), ASSyntax.createNumber(i));
      }
    }
    gui = new HouseGUI();
      ...
      public void run() {
        try {
          while (isRunning()) {
            // add random dirty
            if (r.nextInt(100) < 20) {
              dirty[r.nextInt(n)][r.nextInt(n)] = true;
              gui.paint();
              createPercept();
            }
            Thread.sleep(1000);
    ...
    private void createPercept() {
      ...
      addPercept(ASSyntax.createLiteral("size", ASSyntax.createNumber(n)));

      addPercept(lPos[vcy][vcx]);
    ...
    public boolean executeAction(String ag, Structure action) {
      ...
      try { Thread.sleep(300);}  catch (Exception e) {} // slow down the execution
      ...
      HouseGUI() {
        ...
        for (int j = 0; j < labels.length; j++) {
          for (int i = 0; i < labels.length; i++) {
            ...
            labels[i][j].setPreferredSize(new Dimension(d,d));
        ...
        synchronized (modelLock) { // do not allow changes in the robot location while painting
          for (int i = 0; i < labels.length; i++) {
            for (int j = 0; j < labels.length; j++) {
              ...
              if (vcx == i && vcy == j) {
                l += "<font color=\"red\" size=" + (int)(20/n) + "><b>Robot</b><br></font>";
              }
              if (dirty[i][j]) {
                l += "<font color=\"blue\" size=" + (int)(18/n) + ">*kaka*</font>";
              }
              l += "</center></html>";
